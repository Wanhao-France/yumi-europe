...
function priceUpdate(productSection, priceContainer, getVariant, showSaved) {
  ...
  if (getVariant != undefined) {
    ...
    if (compareAtPrice > price) {
      priceHtml += `<div class="yv-compare-price-box" ttc="${shouldShowTTC}"><span class="yv-visually-hidden">${regularPriceText}</span><span class="yv-product-compare-price no-actualizar"> ${Shopify.formatMoney(
        compareAtPrice,
        moneyFormat
      )}</span>
      ${savedAmountHtml}</div>`;
